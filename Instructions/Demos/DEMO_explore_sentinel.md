...
1. Na janela que será aberta, leia a descrição e revise as informações associadas ao modelo.  Selecione **Criar regra** na parte inferior da página.
    ...
    1. Na guia Resposta automatizada, veja como adicionar um guia estratégico para automatizar a resposta.  Da mesma forma, você pode criar uma regra de automação de incidentes.  Selecione **+ Adicionar** novo em Automação de incidentes.  Uma janela para criar uma nova regra de automação é aberta.  Qualquer regra de automação criada nesta página será acionada pela regra de automação que você está criando. Veja que é possível adicionar condições e definir ações para a regra.   Selecione **Cancelar** para sair da janela.
